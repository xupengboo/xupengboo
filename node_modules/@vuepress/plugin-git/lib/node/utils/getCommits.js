...
const getFormat = ({ contributors, changelog }) => {
...
export const mergeRawCommits = (commits) => {
    ...
    commits.forEach(({ filepath, ...commit }) => {
        ...
        if (_commit)
            ...
            commitMap.set(commit.hash, { filepaths: [filepath], ...commit });
    });
    const result = Array.from(commitMap.values());
    return result;
};
export const getCommits = async (filepaths, cwd, options) => {
    const rawCommits = await Promise.all(filepaths.map((filepath) => getRawCommits(filepath, cwd, options)));
    return mergeRawCommits(rawCommits.flat()).sort((a, b) => b.date - a.date > 0 ? 1 : -1);
};
