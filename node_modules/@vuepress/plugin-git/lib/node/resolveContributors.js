import { digestSHA256, getContributorInfo, getUserNameWithNoreplyEmail, } from './utils/index.js';
export const getRawContributors = (commits, options, gitProvider) => {
    ...
    for (const commit of commits.reverse()) {
        ...
        for (const { name: author, email } of authors) {
            const config = getContributorInfo(getUserNameWithNoreplyEmail(email) ?? author, options.info);
            ...
            if (contributor) {
                ...
                const url = (config?.url ?? gitProvider === 'github')
...
export const resolveContributors = (commits, gitProvider, options, extraContributors = []) => {
    ...
    if (options.info?.length && extraContributors.length) {
        for (const extraContributor of extraContributors) {
            if (contributors.every((item) => item.name !== extraContributor)) {
                const contributorInfo = getContributorInfo(extraContributor, options.info);
                if (!contributorInfo)
                    continue;
                const result = {
                ...
                if (options.avatar)
                    result.avatar =
                        contributorInfo.avatar ??
                            options.avatarPattern?.replace(':username', contributorInfo.username) ??
                            (gitProvider === 'github'
