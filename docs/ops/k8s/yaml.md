...
## 1. kubectl create 命令介绍

::: info
Kubernetes 本身就提供了官方、准确、与你的集群版本100%兼容的获取YAML模板的方法。网上的教程很可能使用过时的API版本或字段，而从集群本身获取的模板永远是正确的。
:::

### 1.1 `kubectl create --dry-run` 快速生成基础模板（最常用）
...
### 1.2 `kubectl explain` 查看完整字段说明（最强大）
...
### 1.3 `kubectl get -o yaml` 导出现有资源配置（最实用）
...
### 6.7 StatefulSet 、PersistentVolume（PV）、PersistentVolumeClaim (PVC)、StorageClass 资源类型

#### 6.7.1 StatefulSet 有状态应用
...
一般像 Redis、MySQL、KafKa 等，这种需要持久化数据和网络标识的构建，需要使用该资源类型。

> **要整明白：`StorageClass`  、 `PersistentVolume (PV)` 、 `PersistentVolumeClaim (PVC)` ，三者的关系**。
>
> -  `volumeClaimTemplates(StatefulSet的yaml配置中的)` = “我要什么样的磁盘”
>
> - `StorageClass` = “怎么创建这种磁盘”
>
> - `PVC` = “磁盘申请单”
>
> - `PV` = “真正创建出来的磁盘资源”
...
#### 6.7.2 StorageClass 的 Yaml 配置解析

1. `reclaimPolicy`：StorageClass的回收策略 
- 会自动设置 `pv` 的 `PV.spec.persistentVolumeReclaimPolicy` 配置。
- **在生产环境中一定要显式设置为 `Retain`  !**

```yaml
reclaimPolicy: Delete
# 当 StorageClass 的reclaimPolicy: Delete时：
#  - 动态供应的 PV（即通过该 StorageClass 自动创建的 PV），在其绑定的 PVC 被删除后，Kubernetes 会自动删除该 PV 对象。
#  - 同时，绝大多数存储插件会自动删除底层的物理存储卷及其上的所有数据。
#  - 这个配置只影响通过该 StorageClass 动态创建的 PV。对于你手动创建的静态 PV，它们有自己独立的persistentVolumeReclaimPolicy字段，不受 StorageClass 的 reclaimPolicy 影响。

reclaimPolicy: Retain
# 当 StorageClass 设置reclaimPolicy: Retain时，所有通过该 StorageClass动态创建的 PV都会自动继承这个回收策略：
# 1. 当 PVC 被删除：
# - PV 不会被自动删除，PV 的状态从Bound变为Released，底层存储卷和所有数据 100% 完整保留。
# - PV 处于 "已释放但未被清理" 的状态，不能直接被新的 PVC 绑定。
# 2. 手动删除 PV 对象：
# - Kubernetes 只会删除 etcd 中的 PV 元数据对象。
# - 绝大多数主流存储插件绝对不会删除底层的物理存储卷和数据。
```

| 操作        | reclaimPolicy: Delete         | reclaimPolicy: Retain                  |
| ----------- | ----------------------------- | -------------------------------------- |
| 删除 PVC    | 自动删除 PV，自动删除底层数据 | PV 变为 Released，数据完整保留         |
| 手动删除 PV | 自动删除底层数据              | 仅删除 K8s 元数据，数据完整保留        |
| 数据安全性  | 低，误删 PVC 即丢失数据       | 极高，必须手动操作底层存储才会删除数据 |

2. `allowVolumeExpansion` ：配置 StorageClass 必须支持扩容。 
...
### 6.10 Ingress 资源 和 Ingress Controller 控制器
...
### 6.11 serviceaccount 以及 Role、RoleBinding、ClusterRole、ClusterRoleBinding资源类型
...
### 6.12 HorizontalPodAutoscaler（HPA）资源类型
...
### 6.15 PodDisruptionBudget (PDB) 资源类型
...
### 7.2 `PersistentVolume (PV)` 和 `PersistentVolumeClaim (PVC)`
...
### 7.3 `volumes` 和 `volumeMounts` 与 `PVC/PV` 的关系和区别
